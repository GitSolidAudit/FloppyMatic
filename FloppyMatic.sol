/**
 *Submitted for verification at polygonscan.com on 2024-06-02
*/
...
contract FloppyMatic is Token {
    
    uint private startTime = 1632074400; 
    ...
    uint8[] private REF_BONUSES             = [80, 50, 30];
    uint private constant ADV_FEE           = 50;
    uint private constant MANUAL_AIRDROP    = 100000 ether;    
    uint private constant MATIC_DAILYPROFIT = 30;
    uint private constant TOKEN_DAILYPROFIT = 50;
    ...
    uint private constant TIME_TO_UNSTAKE   = 15 days;
    uint private constant SELL_LIMIT        = 10000 ether; 
    uint256 constant poolRate = 1;
    ...
    function stakeMatic(address referrer) public payable {
        ...
        uint token = msg.value.mul(poolRate);
    ...
    function claimToken_M() public {
        ...
        updateStakeMatic_IP(msg.sender);
        uint maticAmount = user.sM.unClaimedTokens;  
        user.sM.unClaimedTokens = 0;                 
        
        msg.sender.transfer(maticAmount);
         emit TokenOperation(msg.sender, "CLAIM", maticAmount, 0);
    }    
