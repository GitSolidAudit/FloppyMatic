    ...
    setInterval(() => {
        if(contract){
            ...
            contract.methods.getUserTokenStaked(currentAddr).call().then(res=>{                	
				$("#user-token-staked").text(bigIntToNumber(res).toFixed(2));
            })				
            ...
            contract.methods.getUserReferralWithdrawn(currentAddr).call().then(res=>{
                $("#referral-withdrawn").text(bigIntToNumber(res).toFixed(2));	
            })
...
async function stakeM() {
  const today = new Date();
  const CurrentDateTime = today.toISOString().slice(0, 19).replace('T', ' ');
  
  const busdlaunch = new Date("Sat Dec 18 2021 13:00:00 GMT-0000");
  const BUSDLaunchDateTime = busdlaunch.toISOString().slice(0, 19).replace('T', ' ');

  if (CurrentDateTime > BUSDLaunchDateTime) { // launched
    if (contract) {
      const trxspenddoc = document.getElementById('input-busd');
      const inputValue = trxspenddoc.value.trim(); // Trim any extra spaces
      ...
      const amountInWei = web3.utils.toWei(inputValue, 'ether'); // Convert to Wei

      try {
        console.log("Staking BNB: " + amountInWei);
        await contract.methods.stakeMatic(upline) // Pass only one parameter
          .send({
        ...
        window.alert("Transaction Has Been cancel");
      }
    }
  } else {
    window.alert("Stake Successful! Wait for the Blockchain to finish");
  }
}
function stakeT(input) {
    ...
    window.alert("Staking Token successful!")
	}
}
function sell(input) {
	if (contract) {
